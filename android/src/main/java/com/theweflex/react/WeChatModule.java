...
//    }
    ...
    //分享微信小程序
    @ReactMethod
    public void shareToWXMiniProgram(final ReadableMap data, final Callback callback) throws FileNotFoundException {
        Log.e("TAG", "分享微信小程序");
        if (api == null) {
            callback.invoke(NOT_REGISTERED);
            return;
        ...
        String thumbPath = "";
        if (data.hasKey("thumb")) {
            thumbPath = data.getString("thumb");
        }
        Uri imageUri;
        try {
            imageUri = Uri.parse(thumbPath);
            // Verify scheme is set, so that relative uri (used by static resources) are not handled.
            if (imageUri.getScheme() == null) {
                imageUri = getResourceDrawableUri(getReactApplicationContext(), thumbPath);
            }
        } catch (Exception e) {
            imageUri = null;
        }
        ...
        this._getImage(imageUri, null, new ImageCallback() {
            ...
            public void invoke(@Nullable Bitmap bitmap) {
                String id = "", title = "",thumbPath = "",path = "",userName = "";
                if (data.hasKey("id")) {
                ...
                if (data.hasKey("shareUrl")) {
                    path = data.getString("shareUrl");
                }
                if (data.hasKey("miniUserName")) {
                    userName = data.getString("miniUserName");
                }
                Log.e("小程序图标", thumbPath);

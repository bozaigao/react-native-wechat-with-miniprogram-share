...
//分享微信小程序
RCT_EXPORT_METHOD(shareToWXMiniProgram:(NSDictionary *)aData
                  :(RCTResponseSenderBlock)callback)
{
    WXMiniProgramObject *object = [WXMiniProgramObject object];
    NSString * title = aData[@"title"];
    NSString * thumbPath = aData[@"thumb"];
    NSString * path = aData[@"shareUrl"];
    object.webpageUrl = @"http://www.qq.com";
    object.userName = aData[@"miniUserName"];
    object.path = path;
    if([thumbPath hasPrefix:@"http"]){
        object.hdImageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:thumbPath]];
    }else{
        object.hdImageData = [NSData dataWithContentsOfFile:thumbPath];
    }
//    object.withShareTicket = withShareTicket;
//    object.miniProgramType = UShareWXMiniProgramTypeRelease; // 可选体验版和开发版;
